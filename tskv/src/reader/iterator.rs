...
impl LevelTSDataStream {
    pub async fn new(
        ...
        > = vec![];
        let peeked_file_locations: Vec<Option<FieldFileLocation>> =
            (0..column_files.len()).map(|_| None).collect();
        for file in column_files.into_iter() {
            let path = file.file_path();
            let tsm_reader = version.clone().get_tsm_reader(path).await?;
            ...
            field_file_locations.push(Box::new(res))
        }
        Ok(Self {
...
pub type TsDataStream = Pin<Box<dyn Stream<Item = Result<DataType>> + Send>>;
